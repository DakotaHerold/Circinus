...
public:
	...
	explicit StackAllocator(const allocator_type& alloc = allocator_type())
		: m_allocator(alloc), m_begin(nullptr), m_end(nullptr), m_stack_pointer(nullptr)
	{ }

	explicit StackAllocator(pointer buffer, const allocator_type& alloc = allocator_type())
		: m_allocator(alloc), m_begin(buffer), m_end(buffer + N),
		m_stack_pointer(buffer)
	{ }

	template <class U>
	StackAllocator(const StackAllocator<U, N, Allocator>& other)
	...
	template <class U>
	struct rebind { typedef StackAllocator<U, N, allocator_type> other; };
...
bool operator == (const StackAllocator<T1, N, Allocator>& lhs,
